...
public class SQLTodoRepository implements TodoRepository {
    ...
    public List<Todo> findAllByListId(int listId){
        try (Connection connection = dataSource.getConnection()) {
            PreparedStatement stmt = connection.prepareStatement("SELECT * FROM todos WHERE listId = ?");
    ...
    public int save(Todo todo) {
        try (Connection connection = dataSource.getConnection()) {
            PreparedStatement stmt = connection.prepareStatement(
                    "INSERT INTO todos (description, completed, listId) VALUES (?, ?, ?)",
    ...
    public void update(Todo todo){
        try (Connection connection = dataSource.getConnection()) {
            PreparedStatement stmt = connection.prepareStatement(
                    "UPDATE todos (description, completed, listId) VALUES (?, ?, ?) WHERE id = ?",
                    Statement.RETURN_GENERATED_KEYS);
            stmt.setString(1, todo.getDescription());
            stmt.setBoolean(2, todo.isCompleted());
            stmt.setInt(3, todo.getListId());
            ...
            stmt.executeUpdate();
            //podia retornar um boolean ou o objeto
        } catch (SQLException e) {
            throw new RuntimeException("Failed to save todo", e);
        }
    }
