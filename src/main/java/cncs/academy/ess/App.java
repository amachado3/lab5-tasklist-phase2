...
public class App {
    public static void main(String[] args) throws NoSuchAlgorithmException {
        ...
        String connectURI = String.format("jdbc:postgresql://%s:%s/%s?user=%s&password=%s", "localhost", "5440", "postgres", "postgres", "changeit");
        ...
        TodoUserService todoUserService = new TodoUserService(userRepository);
        TodoController todoController = new TodoController(todoService, toDoListService);

        AuthorizationMiddleware authMiddleware = new AuthorizationMiddleware(userRepository, todoUserService);
